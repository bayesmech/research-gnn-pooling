...
class DiffPooledConvolutionalNetwork(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        s = self.pool1(x)
        x, adj, lp1, er1 = pyg.nn.dense_diff_pool(x, adj, s, mask)
        x = self.conv2(x, adj).relu()
        # Second round of pool-conv
        s = self.pool2(x)
        x, adj, lp2, er2 = pyg.nn.dense_diff_pool(x, adj, s)
        x = self.conv3(x, adj)
        # Final global pool and output
        x = x.mean(dim=1)
        x = self.lin1(x).relu()
        x = self.lin2(x)
        return torch.nn.functional.log_softmax(x, dim=-1), lp1 + lp2 + er1 + er2
